...
def new_tile(
    *, # Enforce the use of keywords, so that param order doesn't matter
    ...
    light: Tuple[int, Color, Color],
) -> np.ndarray:
    """Helper function for defining individual tile types """
    return np.array((walkable, transparent, dark, light), dtype=tile_dt)

# Shroud represents unexplored, unseen tiles.
SHROUD = np.array((ord(" "), sd.BASE0, sd.BASE01), dtype=graphic_dt)

floor = new_tile(
    walkable=True, 
    transparent=True, 
    dark=(ord(" "), sd.BASE0, sd.BASE02),
    light=(ord(" "), sd.BASE0, sd.BASE1),
)

wall = new_tile(
    walkable=False,
    transparent=False, 
    dark=(ord("H"), sd.BASE0, sd.BASE01),
    light=(ord("H"), sd.BASE0, sd.BASE2),
)
